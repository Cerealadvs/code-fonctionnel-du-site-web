...
function getCart() {
  ...
  for (let produit in produitLocalStorage) {
    document.getElementById(
      "cart__items"
    ).innerHTML += `<article class="cart__item" data-color="${produitLocalStorage[produit].couleurProduit}" data-id="${produitLocalStorage[produit].idProduit}">
...
const firstNameCheck = () => {
  ...
  if (!/^[A-Za-zÀ-ÿ\-' ]+$/gi.test(firstName.value) || firstName.value == "") {
    ...
    console.log(
      "le prénom : " + firstName.value + " ne correspond pas au modèle"
    );
...
orderBtn.addEventListener("click", (e) => {
  ...
  if (
    ...
    e.preventDefault();
    alert(
      "Veuillez suivre les instructions pour remplir le formulaire correctement."
    );
  } else {
