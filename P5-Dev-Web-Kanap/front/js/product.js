...
fetch(`http://localhost:3000/api/products/${id}`)
  ...
  .catch(function (error) {
    ...
    alert(
      "Desolé nous ne sommes pas en mesure de vous presenter nos produit, reessayer ulterieurement."
    );
  });
  ...
  btn_envoyerPanier.addEventListener("click", function (event) {
    ...
    if (
      ...
      const confirmationPannier = function () {
        ...
        if (
          window.confirm(
            `Votre commande de ${choixQuantite} ${product.name} ${choixCouleur} est ajoutée au panier`
          )
        ) {
